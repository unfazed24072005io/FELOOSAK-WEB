...
router.post("/", async (req, res) => {
  try {
    if (!req.session.userId) return res.status(401).json({ error: "Not authenticated" });
    const { customerId, invoiceNo, status, subtotal, vatAmount, total, terms, billingAddress, notes, items, invoiceDate } = req.body;
    ...
    const [invoice] = await db.insert(invoicesTable).values({
      ...
      subtotal: (subtotal || 0).toString(), vatAmount: (vatAmount || 0).toString(),
      total: (total || 0).toString(), terms: terms || "net30",
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const { status, subtotal, vatAmount, total, terms, billingAddress, notes, items } = req.body;
